...
const Assignments = ({ searchQuery }) => {
    ...
    const { data: assignments, isLoading } = useQuery({
        ...
        queryFn: async () => {
            const res = await assignmentApi.getAllAssignments(pagination.page + 1, pagination.rows, searchQuery);
            return res.data;
        }
    });
    ...
    return (
        <div className='mt-10'>
            ...
            {!isLoading && (Array.isArray(assignments?.rows) && assignments?.rows?.length > 0 ?
                <div>
                    <div className='grid sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-5 max-sm:px-5'>
                        {assignments?.rows.map(item => <div className='bg-white flex flex-col p-3 rounded-lg shadow-lg' key={item?.id}>
                            ...
                            <div className='flex justify-between pt-2 border-[#0000000F] border-t'>
                                <div className='flex flex-col gap-y-2'>
                                    <p className='flex flex-col'>
                                        <span className='text-base 2xl:text-xl font-medium text-[#00285A]'>Status</span>
                                        <span className='text-[#00000099] text-sm 2xl:text-lg'>
                                            {item?.submissions?.status === 'submitted' ? (item?.submissions?.evaluated ? 'Evaluated' : 'Submitted') : 'Not Complete'}
                                        </span>
                                    </p>
                                    <p className='flex flex-col'>
                                        <span className='text-base 2xl:text-xl font-medium text-[#00285A]'>Marks</span>
                                        <span className='text-[#00000099] text-sm 2xl:text-lg'>
                                            {item?.submissions?.id ? (item?.submissions?.evaluated ? item?.submissions?.scores : '') : 'Not Done'}
                                        </span>
                                    </p>
                                </div>
                                <div className='flex flex-col gap-y-2'>
                                    <p className='flex flex-col'>
                                        ...
                                        <span className='text-[#00000099] text-sm 2xl:text-lg'>{moment(item.createdAt).format('ll')}</span>
                                    </p>
                                    <p className='flex flex-col'>
