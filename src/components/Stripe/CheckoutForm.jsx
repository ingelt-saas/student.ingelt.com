...
const MasterCardSVG = ({ className, active }) => (
    ...
    >
        <rect width={780} height={500} fill={active ? '#16366F' : '#fff'} />
        <path
...
const PaymentMethod = ({ selectedMethod, method, onChange, label, children }) => {

    const isExpand = method && selectedMethod === method;

    return (
        <Accordion
            expanded={isExpand}
            onChange={() => onChange(method)}
            className={`!duration-300 ${isExpand ? '!bg-[#fff] !shadow-lg' : '!bg-[#0C3C82]'}`}
        >
        ...
        <form id='payment-form' onSubmit={()=>console.log('error in payment')} className='py-5'>
            ...
            <div className='mb-3'>
                <PaymentMethod
                    label={'Pay with Card'}
                    method={'card'}
                    onChange={(e) => setSelectedMethod(e === selectedMethod ? '' : e)}
